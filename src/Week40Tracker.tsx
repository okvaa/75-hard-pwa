...
const injectStyles = () => {
  ...
  style.textContent = `
    :root {
      ...
      --red: #ef4444; --red-dim: rgba(240,68,68,0.1);
    ...
    * { box-sizing: border-box; }
    ...
    .hint-delta.deload { background: rgba(240,68,68,0.15); color: var(--red); }
    ...
    .hint-apply-btn.deload { border-color: rgba(240,68,68,0.3); color: var(--red); }
    .hint-apply-btn.deload:hover { background: rgba(240,68,68,0.08); border-color: var(--red); }
...
const week40Plan: PlanDay[] = [
  {
    date: "Mon March 30 • Week 40",
  ...
  },
  {
    ...
    exercises: [
      ...
      { name: "Incline Treadmill Walk", sets: 1, reps: "15 min" },
    ],
  },
  {
    date: "Thu April 2 • Week 40",
  ...
  {
    date: "Sat April 4 • Week 40",
    label: "CONDITIONING + Core",
    exercises: [
      { name: "Sauna", sets: 1, reps: "15 min" },
      { name: "Incline Treadmill Walk", sets: 1, reps: "40-45 min" },
...
const logKey = (week: number) => `w40log_w${week}`;
const recKey = (week: number, dayLabel: string) => `w40rec_w${week}_${dayLabel}`;
...
export default function Week40Tracker() {
  ...
  const weekPlan = useMemo(() => week40Plan, []);
