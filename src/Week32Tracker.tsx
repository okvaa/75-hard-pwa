...
const week32Plan = [/* ⬅️ unchanged, keep your plan exactly as-is */
  {
    date: "Mon • Feb 2",
    label: "PUSH",
    exercises: [
...
export default function Week32Tracker() {
  const [dayIndex, setDayIndex] = useState(0);
  const [log, setLog] = useState<Record<string, DayLog>>({});

  const day = week32Plan[dayIndex];

  const hasData = Object.keys(log).length > 0;

  /* Load from localStorage */
  useEffect(() => {
    const stored = localStorage.getItem("week32Log");
    if (stored) setLog(JSON.parse(stored));
  }, []);

  /* Save to localStorage */
  useEffect(() => {
    localStorage.setItem("week32Log", JSON.stringify(log));
  }, [log]);
  ...
  a.href = url;
  a.download = "week32-training-log.json";
  a.click();
...
const downloadCSV = () => {
  ...
  const a = document.createElement("a");
  a.href = url;
  a.download = "week32-training-log.csv";
  a.click();
  URL.revokeObjectURL(url);
};
 /* ================= COMPLETION ================= */
  ...
  return (
    <ScrollArea className="max-w-md mx-auto p-4 space-y-4">
      ...
      <div className="text-center font-bold text-lg">
        Week of Feb 3 – Feb 8 • Week 32
      </div>
      <div className="text-center font-bold text-lg">Week 32 Training</div>
      ...
      <div className="flex justify-between items-center">
        ...
        <Button
          disabled={dayIndex === week32Plan.length - 1}
