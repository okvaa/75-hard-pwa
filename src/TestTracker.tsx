...
export default function TestTracker() {
  ...
  const day = week00Plan[dayIndex];

  /* Load from localStorage */
  useEffect(() => {
    const stored = localStorage.getItem("week00Log");
    if (stored) setLog(JSON.parse(stored));
  }, []);

  /* Save to localStorage */
  useEffect(() => {
    localStorage.setItem("week00Log", JSON.stringify(log));
  }, [log]);
  ...
  a.href = url;
  a.download = "week00-training-log.json";
  a.click();
...
const downloadCSV = () => {
  ...
  const a = document.createElement("a");
  a.href = url;
  a.download = "week00-training-log.csv";
  a.click();
  URL.revokeObjectURL(url);
};
   /* Completion logic */
...
/* ================= HEADER ================= */
  ...
  return (
    <ScrollArea className="max-w-md mx-auto p-4 space-y-4">
      ...
      <div className="text-center font-bold text-lg">Week ??? Training</div>
      ...
      <div className="flex justify-between items-center">
        ...
        <Button
          disabled={dayIndex === week00Plan.length - 1}
