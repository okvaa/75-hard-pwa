...
export default function MainTaskMenu() {
  ...
  return (
    <div className="p-4 space-y-4 max-w-sm mx-auto">
      <h2 className="text-lg font-bold mb-2">Choose a Task</h2>
      ...
      {selectedTask === "Read 10 pages" && (
        <div className="mt-6 space-y-2">
          <p className="text-sm">Pages read: {pagesRead}</p>
          <Button onClick={handlePagesIncrease}>Add 1 Page</Button>
          <Button variant="outline" onClick={resetProgress}>Reset</Button>
        </div>
      )}

      {selectedTask === "Take a progress photo" && (
        <div className="mt-6 space-y-2">
          <p className="text-sm">Tap to upload today's photo:</p>
      ...
      {selectedTask === "Follow a diet" && (
        <div className="mt-6">
          <p className="text-sm">Stay consistent and no cheat meals!</p>
        </div>
      )}
    </div>
  );
