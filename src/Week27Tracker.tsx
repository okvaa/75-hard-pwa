...
export default function Week27Tracker() {
  const [dayIndex, setDayIndex] = useState(0);
  const [log, setLog] = useState<Record<string, DayLog>>({});

  const day = week27Plan[dayIndex];

  useEffect(() => {
    const stored = localStorage.getItem("week27Log");
    if (stored) setLog(JSON.parse(stored));
  }, []);

  useEffect(() => {
    localStorage.setItem("week27Log", JSON.stringify(log));
  }, [log]);
  ...
  return (
    <ScrollArea className="max-w-md mx-auto p-4 space-y-4">
      ...
      <div className="flex justify-between items-center">
        ...
        <Button
          variant="outline"
          disabled={dayIndex === week27Plan.length - 1}
          onClick={() => setDayIndex((i) => i + 1)}
        >
          ▶
        </Button>
