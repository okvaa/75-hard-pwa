...
export default function Week33Tracker() {
  ...
  const updateSet = (
    ...
    setLog((prev) => {
      ...
      const existingSets = entry.sets ?? [];

      const sets = [...existingSets];

    // Ensure set exists before updating
      sets[i] = {
      ...(sets[i] ?? {}),
      [field]: value,
     };
          ...
          {day.exercises.map((e) => {
           const entry: ExerciseLog = {
             ...
             target: e.target || "",
              machine: "",
              completed: false,
             ...log[day.label]?.[e.name], // override with existing saved log if any
          };
            ...
            {/* Cardio checkbox */}
             ...
             {/* Strength */}
              {!isCardioExercise(e.name) && (
                 <><Input
                       placeholder="Target (RIR / Load / Focus)"
                       value={entry?.target || ""}
                      onChange={(ev) =>
                       setLog((p) => ({
                          ...p,
