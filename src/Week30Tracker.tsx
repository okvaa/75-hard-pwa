...
export default function Week30Tracker() {
  const [dayIndex, setDayIndex] = useState(0);
  const [log, setLog] = useState<Record<string, DayLog>>({});

  const day = week30Plan[dayIndex];

  useEffect(() => {
    const stored = localStorage.getItem("week30Log");
    if (stored) setLog(JSON.parse(stored));
  }, []);

  useEffect(() => {
    localStorage.setItem("week30Log", JSON.stringify(log));
  }, [log]);
  ...
  const updateSet = (
    ...
    setLog((prev) => {
      ...
      const sets =
        existing?.sets ??
        Array.from({ length: day.exercises.find(e => e.name === ex)!.sets }, () => ({}));
  ...
  return (
     <ScrollArea className="max-w-md mx-auto p-4 space-y-4">
      ...
      <div className="flex justify-between items-center">
        ...
        <Button
          variant="outline"
          disabled={dayIndex === week30Plan.length - 1}
          onClick={() => setDayIndex(i => i + 1)}
        >
          ▶
        </Button>
