...
const week34Plan = [/* ⬅️ unchanged, keep your plan exactly as-is */
  {
    date: "Mon • Feb 15",
    label: "PUSH",
    exercises: [
...
export default function Week34Tracker() {
  const [dayIndex, setDayIndex] = useState(0);
  const [log, setLog] = useState<Record<string, DayLog>>({});

  const day = week34Plan[dayIndex];

  const hasData = Object.keys(log).length > 0;

  /* Load from localStorage */
  useEffect(() => {
    const stored = localStorage.getItem("week34Log");
    if (stored) setLog(JSON.parse(stored));
  }, []);

  /* Save to localStorage */
  useEffect(() => {
    localStorage.setItem("week34Log", JSON.stringify(log));
  }, [log]);
  ...
  a.href = url;
  a.download = "week34-training-log.json";
  a.click();
...
const downloadCSV = () => {
  ...
  const a = document.createElement("a");
  a.href = url;
  a.download = "week34-training-log.csv";
  a.click();
  URL.revokeObjectURL(url);
};
 /* ================= COMPLETION ================= */
  ...
  return (
    <ScrollArea className="max-w-md mx-auto p-4 space-y-4">
      ...
      <div className="text-center font-bold text-lg">
        Week of Feb 15 – Feb 21 • Week 34
      </div>
      <div className="text-center font-bold text-lg">Week 34 Training</div>
      ...
      <div className="flex justify-between items-center">
        ...
        <Button
          disabled={dayIndex === week34Plan.length - 1}
